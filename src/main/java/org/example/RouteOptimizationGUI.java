...
public class RouteOptimizationGUI extends JFrame {
        ...
        optimizeButton.addActionListener(e -> {
            if (startCity != null && endCity != null && !startCity.equals(endCity)) {
                ...
                if (shortestPath != null && !shortestPath.isEmpty()) {
                    ...
                    pathLabel.setText("No path exists between selected cities.");
                }
                graphPanel.repaint();
            } else {
    ...
    private void initializeCitiesAndDistances() {
        ...
        cityPositions.put("City A", new Point(200, 100));
        cityPositions.put("City B", new Point(100, 300));
        cityPositions.put("City C", new Point(400, 150));
        cityPositions.put("City D", new Point(300, 400));
        cityPositions.put("City E", new Point(500, 300));
        cityPositions.put("City F", new Point(600, 200));

        // Initialize cities and their indices
        String[] cities = {"City A", "City B", "City C", "City D", "City E", "City F"};
        ...
        addConnection("City A", "City B", 200);
        addConnection("City B", "City D", 50);
        addConnection("City D", "City F", 150);
        addConnection("City F", "City E", 50);
        addConnection("City C", "City F", 55);
        addConnection("City C", "City B", 200);
    }
    ...
    private List<Integer> dijkstra(int start, int end) {
        ...
        while (!queue.isEmpty()) {
            int u = queue.poll();
            if (u == end) break;  // Stop early if we reach the end city
            visited[u] = true;
        ...
        return (path.size() > 1 || path.get(0) == end) ? path : null;
    }

    private String shortestPathToString() {
        if (shortestPath == null) return "No path exists.";
